...
const projectsSlice = createSlice({
  ...
  reducers: {
    ...
    moveTask: (
      ...
      if (fromBacklog) {
        const sprintIndex = project.sprints.findIndex(
          (sprint) => sprint.id === sprintIdFrom
        );
        const sprint = project.sprints[sprintIndex];

        const taskIndexFromBacklog = project.backlog.findIndex(
          (task) => task.id === taskId
        );
        ...
        const sprint = project.sprints[sprintIndex];
        const taskIndexFromTodoSprint = sprint.todo.findIndex(
          (task) => task.id === taskId
        );
        const taskIndexFromDoingSprint = sprint.doing.findIndex(
          (task) => task.id === taskId
        );
        const taskIndexFromDoneSprint = sprint.done.findIndex(
          (task) => task.id === taskId
        );
      ...
      if (sprintIdTo) {
        const sprintIndexTo = project.sprints.findIndex(
          (sprint) => sprint.id === sprintIdTo
        );
    ...
    deleteTask: (state, action) => {
      ...
      const projectIndex = state.findIndex((project) =>
        project.backlog.some((task) => task.id === taskId)
      );
      if (projectIndex !== -1) {
        state[projectIndex].backlog = state[projectIndex].backlog.filter(
          (task) => task.id !== taskId
        );
      } else {
        state.forEach((project) => {
          const sprint = project.sprints.find(
            (sprint) =>
              sprint.todo.some((task) => task.id === taskId) ||
    ...
    deleteSprint: (state, action) => {
      ...
      if (projectIndex !== -1) {
        state[projectIndex].sprints = state[projectIndex].sprints.filter(
    ...
    updateTask: (state, action) => {
      ...
      if (projectIndex !== -1) {
        const backlogIndex = state[projectIndex].backlog.findIndex(
          (backlogTask) => backlogTask.id === task.id
        );
        if (backlogIndex !== -1) {
          state[projectIndex].backlog[backlogIndex] = task;
        } else {
          const sprint = state[projectIndex].sprints.find(
          ...
          if (sprint) {
            ...
            const doingIndex = doing.findIndex(
              (doingTask) => doingTask.id === task.id
            );
            const doneIndex = done.findIndex(
    ...
    updateSprint: (state, action) => {
      ...
      if (projectIndex !== -1) {
        const sprintIndex = state[projectIndex].sprints.findIndex(
          (p) => p.id === sprint.id
        );
        if (sprintIndex !== -1) {
